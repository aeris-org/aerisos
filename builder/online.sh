...
cache_dir=$(realpath --canonicalize-missing ~/.cache/iso_$(date +%Y-%m-%d))
mkdir -p $cache_dir
cd $cache_dir
...
curl -fsSL -o "airootfs/root/configurator" \
  "https://raw.githubusercontent.com/$CONFIGURATOR_REPO/$CONFIGURATOR_REF/configurator"
...
mkarchiso -v -w "$cache_dir/work/" -o "/release/" "$cache_dir/"

# Fix permissions on output ISO(s)
chmod 644 /release/*.iso
ls -l /release/
